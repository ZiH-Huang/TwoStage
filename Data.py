...
class DataLayer(data.Dataset):
    def __init__(self, data_path, current_fold, organ_number, low_range, high_range, \
                 slice_threshold, slice_thickness, organ_ID, plane, testing=False):
        self.low_range = low_range
        self.high_range = high_range
        self.slice_thickness = slice_thickness
        self.organ_ID = organ_ID

        image_list = open(training_set_filename(current_fold), 'r').read().splitlines()   # image_list =  N case
        self.training_image_set = np.zeros((len(image_list)), dtype=np.int32)
        ...
        self.image_ID = np.zeros((self.slices), dtype=np.int32)
        self.slice_ID = np.zeros((self.slices), dtype=np.int32)
        ...
        self.pixels = np.zeros((self.slices), dtype=np.int32)
        ...
        for l in range(self.slices):
            ...
            self.average[l] = float(s[5])  # pixel value avg
            self.pixels[l] = int(s[organ_ID * 6])  # sum of label
        ...
        if not testing:
            ...
            self.active_index = [l for l, p in enumerate(self.pixels)
                                 if p >= min_pixels and self.image_ID[l] not in self.training_image_set]  # true active
